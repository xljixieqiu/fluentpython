...
Rusult=namedtuple('Rusult','status data')
...
def process_args(default_concur_req):
	...
	parser.add_argument('-e','--every',action='store_true',help='get flags tor every possible code(AA to ZZ)')
	...
	if args.max_req<1:
		...
		sys.exit(1)
	if args.limit<1:
		print('***Usage error: --limit N must b >=1')
		parser.print_usage()
		sys.exit(1)
...
def main(download_many,default_concur_req,max_concur_req):
	args,cclist=process_args(default_concur_req)
	actual_req =min(args.max_req,max_concur_req,len(cc_list))
	initial_report(cc_list,actual_req,args.server)
	base_url=SERVERS[largs.server]
	t0=time.time()
